...
public class Minesweeper {
    ...
    private void initEmptyBoard() {
        this.data = new Status[this.size_c][this.size_r]; 
    ...
    public void setSize_C_R(int size_c, int size_r, int bomb_n){
        this.size_c = size_c;
        this.size_r = size_r;
        this.bomb_n = bomb_n;
    }
    ...
    public void setStatusAt(int c, int r, MPOS mpos) { 

        if (this.roundStatus != RoundStatus.INGAME)
            return;

        if (mpos == MPOS.RIGHT_CLICK){
    ...
    private int getCountBombAround(int c, int r){
        ...
        for (int temp_c = c - 1; temp_c <= c + 1; temp_c++) {
            for (int temp_r = r - 1; temp_r <= r + 1; temp_r++) { 
                ...
                if (data[temp_c][temp_r] == Status.BOMB || data[temp_c][temp_r] == Status.BOMB_UNDER_FLAG){
                    ++bomb_n;
                }
            }
        }
    ...
    public void scanBomb(int c, int r){   
        for (int temp_c = c - 1; temp_c <= c + 1; temp_c++) {
            for (int temp_r = r - 1; temp_r <= r + 1; temp_r++) { 
                ...
                // Skip flagged cells b/c we do not want to turn flag to something else
                if (cellStatus == Status.FLAG || cellStatus == Status.BOMB_UNDER_FLAG) continue;

                //We onlt check if cells are not opened
    ...
    //         System.out.print(i + " ");
    //     }
    //     System.out.printf(" Column %n"); // Change "Row" to "Column"
